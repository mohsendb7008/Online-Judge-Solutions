...
int ts, from_time, f, facility_price[13], needed_desk;
string not_available = "no desk available";

inline bool is_subset(const string& whole, const string& part){
    for(int i = 0; i < min(whole.size(), part.size()); i++)
        if(whole[i] == '0' && part[i] == '1')
            return false;
    return true;
}
    ...
    for(int i = 0; i < n; i++)
        for(int j = 0; j < number_of_desks[i]; j++){
            if(wanted_type != desk_type[i][j])
                break;
            if(check_facility && !is_subset(desk_facilities[i][j], reserve_facilities))
                continue;
